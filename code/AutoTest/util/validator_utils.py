...
def is_email_col(dist_val, ratio):
    ...
    return len([val for val in dist_val if validators.email(val)]) >= ratio * len(dist_val)

def is_ip_col(dist_val, ratio):
    if any([utils.contains_non_alphabet(val) for val in dist_val]): return False
    return len([val for val in dist_val if validators.ip_address.ipv4(val)]) >= ratio * len(dist_val)

def is_url_col(dist_val, ratio):
    if any([utils.contains_non_alphabet(val) for val in dist_val]): return False
    return len([val for val in dist_val if validators.url(val)]) >= ratio * len(dist_val)

# def is_date_col(dist_val, ratio):
#     if any([utils.contains_non_alphabet(val) for val in dist_val]): return False
#     # if sum([":" in val for val in dist_val]) < 0.8 * len(dist_val): return False
...
def validate(val, t):
    ...
    if t == 'email':
        ...
        return False if not validators.ip_address.ipv4(val) else True
    elif t == 'url':
        return False if not validators.url(val) else True
            
def get_matching_rows(df, pre):
    assert pre[0] == 'validator' and pre[1] in ['email', 'url', 'ip']
    ratio = pre[2]
    if len(df) == 0:
        return df
    if pre[1] == 'email':
...
def compute_cohenh_email(df: pd.DataFrame, matching_idx_dict: dict, pre: list):
    assert pre[0] == 'validator' and pre[1] == 'email'
...
def compute_cohenh_ip(df: pd.DataFrame, matching_idx_dict: dict, pre: list):
    assert pre[0] == 'validator' and pre[1] == 'ip'
...
def compute_cohenh_url(df: pd.DataFrame, matching_idx_dict: dict, pre: list):
    assert pre[0] == 'validator' and pre[1] == 'url'
