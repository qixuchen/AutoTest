...
def prepare_feature_extraction():
    ...
    word_embedding_file = "./sherlock-project/sherlock/features/glove.6B.50d.txt"
    ...
    second_paragraph_vector_file = (
        "./sherlock-project/sherlock/features/par_vec_trained_400.pkl.trainables.syn1neg.npy"
    )
...
def extract_features_non_write_to_file(data: Union[pd.DataFrame, pd.Series]):
    """Extract features from raw data.
        ...
        if first_keys is None:
            ...
            if first_keys_str != keys:
                ...
                for idx, k1 in enumerate(first_keys):
                    ...
                    if k1 != k2:
                        print(f"{k1} != {k2}")
        df_str += (",".join(map(str, features.values())) + "\n")
    return df_str