...
def categorize_features() -> dict:
    ...
    for feature_set in ["char", "word", "par", "rest"]:
        feature_cols_dict[feature_set] = pd.read_csv(
            f"./sherlock-project/sherlock/features/feature_column_identifiers/{feature_set}_col.tsv",
            sep="\t",
            index_col=0,
            header=None,
            squeeze=True,
...
def _get_categorical_label_encodings(y_train, y_val, model_id: str) -> (list, list):
    ...
    np.save(f"./sherlock-project/model_files/classes_{model_id}.npy", encoder.classes_)
